...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        system_prompt = """You are a knowledgeable assistant that finds authentic, verified quotes from famous authors.
...
4. Format your response as a JSON array with exactly 5 quotes"""

        user_prompt = f"""Find me 5 authentic quotes by {author}.
  ...
  {{
    ...
    "confidence": "high/medium/low"
        ...
        if response.status_code == 200:
            ...
            # Parse the response
            candidate_quotes = parse_quotes_from_response(content)
...
def parse_quotes_from_response(content: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Ensure we have a list and limit to 5 quotes
        if not isinstance(candidate_quotes, list):
            candidate_quotes = []
        else:
            candidate_quotes = candidate_quotes[:5]
